...
THIN_BIRDNET_SERVICE="thin-birdnet-flac.service"
...
reconcile_service_state() {
    if birdnet_enabled; then
        ...
        systemctl enable "${THIN_BIRDNET_SERVICE}"
        log "enabled ${BIRDNET_SERVICE}"
        log "enabled ${THIN_BIRDNET_SERVICE}"
    else
        systemctl disable --now "${BIRDNET_SERVICE}" >/dev/null 2>&1 || true
        systemctl disable --now "${THIN_BIRDNET_SERVICE}" >/dev/null 2>&1 || true
        log "BirdNET not enabled; disabled ${BIRDNET_SERVICE}"
        log "BirdNET not enabled; disabled ${THIN_BIRDNET_SERVICE}"
    fi
}
